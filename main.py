...
def init_db():
    ...
    # file_path ustuni bilan to'g'ri support_messages jadvali
    ...
    # Mavjud support_messages jadvaliga file_path ustunini qo'shish (migration)
    try:
        c.execute('ALTER TABLE support_messages ADD COLUMN file_path TEXT')
    except Exception:
...
def render_page(body_content: str, **kwargs) -> str:
    ...
    aria_math_url = "/static/music/bg.mp3"
...
main {{opacity:0;transform:translateY(20px);animation:pageLoad 0.5s cubic-bezier(0.4,0,0.2,1) forwards;animation-delay:0.1s;}}
...
.status-card.pulse-green .status-icon{{animation:statusPulse 2s ease-in-out infinite;}}
...
.status-icon{{width:40px;height:40px;border-radius:11px;display:flex;align-items:center;justify-content:center;font-size:1.1rem;flex-shrink:0;}}
...
.status-info .status-label{{display:block;font-size:.68rem;color:var(--text-dim);text-transform:uppercase;letter-spacing:1.2px;font-weight:600;margin-bottom:.12rem;}}
.status-info .status-value{{display:block;font-family:'Orbitron',sans-serif;font-size:1rem;font-weight:700;color:#fff;}}
...
.rank-chip{{font-family:'Orbitron',sans-serif;font-size:.6rem;font-weight:700;padding:.28rem .7rem;border-radius:20px;letter-spacing:.8px;text-transform:uppercase;border:1px solid;transition:var(--transition);cursor:default;}}
...
.btn-danger{{background:linear-gradient(135deg,#ff3366,#e11d48);color:#fff;}}
...
.stat-card{{background:var(--glass);backdrop-filter:blur(14px);border:1px solid rgba(0,255,136,.12);border-radius:var(--radius);padding:2rem 1.5rem;text-align:center;transition:var(--transition);}}
.stat-card:hover{{transform:translateY(-6px);border-color:var(--primary);box-shadow:var(--glow-green);}}
.stat-card i{{font-size:2rem;color:var(--primary);margin-bottom:.6rem;display:block;}}
.stat-card h3{{font-family:'Orbitron',sans-serif;font-size:1.8rem;color:var(--primary);margin:.4rem 0;}}
.stat-card p{{color:var(--text-dim);font-size:.9rem;font-weight:500;}}
...
.package-name{{font-family:'Orbitron',sans-serif;font-size:1.5rem;font-weight:900;text-align:center;margin-bottom:.3rem;}}
.package-desc{{text-align:center;color:var(--text-dim);font-size:.9rem;margin-bottom:1rem;}}
.package-price{{font-family:'Orbitron',sans-serif;font-size:2rem;font-weight:900;text-align:center;color:var(--primary);margin:1rem 0;}}
.package-price span{{font-size:.85rem;color:var(--text-dim);font-weight:400;font-family:'Rajdhani',sans-serif;}}
.package-features{{list-style:none;margin:1rem 0;flex-grow:1;}}
.package-features li{{padding:.45rem 0;border-bottom:1px solid rgba(255,255,255,.05);display:flex;align-items:center;gap:.6rem;font-size:.9rem;color:var(--text-dim);}}
...
table thead th{{padding:1rem 1.2rem;text-align:left;color:var(--primary);font-weight:700;font-size:.85rem;text-transform:uppercase;letter-spacing:.8px;border-bottom:1px solid rgba(0,255,136,.2);white-space:nowrap;}}
...
.balance-hero{{background:linear-gradient(135deg,rgba(168,85,247,.25),rgba(255,0,153,.2));border:1px solid rgba(168,85,247,.3);border-radius:var(--radius-lg);padding:2.5rem;text-align:center;margin:1.5rem 0;}}
.balance-hero h3{{color:var(--text-dim);font-size:1rem;margin-bottom:.5rem;text-transform:uppercase;letter-spacing:1px;}}
.balance-amount{{font-family:'Orbitron',sans-serif;font-size:2.8rem;font-weight:900;color:#fff;}}
...
.tab{{padding:.6rem 1.2rem;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:var(--radius-sm);color:var(--text-dim);text-decoration:none;font-weight:600;font-size:.88rem;transition:var(--transition);display:flex;align-items:center;gap:.4rem;cursor:pointer;}}
...
/* Chat input area */
.chat-input-area{{display:flex;gap:.5rem;padding-top:1rem;border-top:1px solid rgba(255,255,255,.07);margin-top:auto;align-items:center;}}
.chat-input-area input[type=text]{{flex-grow:1;padding:.75rem 1rem;background:rgba(255,255,255,.04);border:1.5px solid rgba(255,255,255,.1);border-radius:var(--radius-sm);color:var(--text);font-size:.9rem;}}
.chat-input-area input[type=text]:focus{{outline:none;border-color:var(--primary);box-shadow:var(--glow-green);}}
/* File attach button */
.attach-btn{{width:42px;height:42px;border-radius:var(--radius-sm);background:rgba(255,255,255,.05);border:1.5px solid rgba(255,255,255,.1);color:var(--text-dim);cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:1rem;transition:var(--transition);flex-shrink:0;position:relative;overflow:hidden;}}
.attach-btn:hover{{border-color:var(--primary);color:var(--primary);background:rgba(0,255,136,.08);}}
.attach-btn input[type=file]{{position:absolute;inset:0;opacity:0;cursor:pointer;width:100%;height:100%;}}
/* Chat image preview */
.chat-img-preview{{max-width:220px;max-height:180px;border-radius:10px;border:2px solid rgba(0,255,136,.35);margin-top:.5rem;display:block;cursor:pointer;transition:var(--transition);}}
.chat-img-preview:hover{{border-color:var(--primary);transform:scale(1.02);}}
/* Upload progress */
.upload-progress{{display:none;align-items:center;gap:.5rem;padding:.4rem .8rem;background:rgba(0,255,136,.08);border:1px solid rgba(0,255,136,.2);border-radius:var(--radius-sm);font-size:.82rem;color:var(--primary);}}
.upload-progress.show{{display:flex;}}
.upload-progress .spinner{{width:14px;height:14px;border:2px solid rgba(0,255,136,.3);border-top-color:var(--primary);border-radius:50%;animation:spin .7s linear infinite;}}
...
if (_newsForm) {{
    _newsForm.addEventListener('submit', async (e) => {{
        ...
        try {{
            const r = await fetch('/admin/add_news', {{ method: 'POST', body: formData }});
            const j = await r.json();
            showToast(j.message, j.success ? 'success' : 'error');
...
def add_news():
    ...
    try:
        conn = get_db()
        conn.execute('INSERT INTO news (title, content, image) VALUES (?, ?, ?)', (title, content, image_url))
        conn.commit()
        conn.close()
...
def shop():
    ...
    <div class="card" style="grid-column:1/-1;max-width:600px;margin:0 auto;border-color:#fbbf24;">
        ...
        <div style="padding:1rem;">
            ...
            ''' + (
        '<button class="btn btn-primary btn-full" onclick="buyCustomTokens()">Sotib Olish</button>' if 'user_id' in session else '<a href="/login" class="btn btn-primary btn-full">Kirish Kerak</a>') + '''
        </div>
...
def rules():
    ...
    <div class="container" style="padding-top:2rem;padding-bottom:3rem;">
        ...
        <div class="card" style="margin-bottom:2rem;">
            <div class="card-header"><i class="fas fa-book-open"></i><h2>Server Qoidalari</h2></div>
            <div style="display:flex;flex-direction:column;gap:15px;font-size:1.1rem;color:var(--text);">
                <div style="padding:12px 15px;background:rgba(255,51,102,.1);border-left:5px solid #ff3366;border-radius:8px;display:flex;justify-content:space-between;align-items:center;">
                    <span><b>SO'KINISH</b></span><span style="color:var(--text-dim);">2 SOAT MUTE 🔕</span>
                </div>
                <div style="padding:12px 15px;background:rgba(0,255,136,.1);border-left:5px solid var(--primary);border-radius:8px;display:flex;justify-content:space-between;align-items:center;">
                    <span><b>CHEATERLIK</b></span><span style="color:var(--text-dim);">2 KUN BAN ✨</span>
                </div>
                <div style="padding:12px 15px;background:rgba(255,170,0,.1);border-left:5px solid var(--warning);border-radius:8px;display:flex;justify-content:space-between;align-items:center;">
                    <span><b>REKLAMA QILISH</b></span><span style="color:var(--text-dim);">1 KUN MUTE 🔕</span>
                </div>
                <div style="padding:12px 15px;background:rgba(168,85,247,.1);border-left:5px solid var(--accent2);border-radius:8px;display:flex;justify-content:space-between;align-items:center;">
                    <span><b>JINSIY MOTERALLAR (CHAT)</b></span><span style="color:var(--text-dim);">6 SOAT MUTE ⚡</span>
                </div>
                <div style="padding:12px 15px;background:rgba(0,153,255,.1);border-left:5px solid var(--secondary);border-radius:8px;display:flex;justify-content:space-between;align-items:center;">
                    <span><b>YOLG'ON MA'LUMOT TARQATISH</b></span><span style="color:var(--text-dim);">10 KUN BAN ✨</span>
                </div>
            </div>
        </div>
        <div class="card" style="border:2px solid var(--warning);background:rgba(255,170,0,.05);">
            <div class="card-header"><i class="fas fa-star" style="color:var(--warning);"></i><h2 style="color:var(--warning);">⭐ HELPER YOKI MODER UCHUN</h2></div>
            <div style="display:flex;flex-direction:column;gap:15px;font-size:1.1rem;">
                <div style="padding:10px;border:1px dashed var(--warning);border-radius:8px;color:#fff;">❌ <b>BESABAB MUTE</b> - ISHDAN OZOD QILINADI ‼️</div>
                <div style="padding:10px;border:1px dashed var(--warning);border-radius:8px;color:#fff;">❌ <b>BESABAB BAN</b> - ISHDAN OZOD QILINADI ‼️</div>
            </div>
        </div>
    </div>'''
    return render_page(content, logged_in='user_id' in session, is_admin=session.get('is_admin', False))
...
def profile():
    ...
    <div class="container" style="max-width:1100px;margin:0 auto;padding-top:2rem;">
        ...
        <div class="card">
            ...
            <div style="padding:.8rem;background:rgba(255,255,255,.03);border-radius:8px;display:flex;justify-content:space-between;">
                <span>Balans</span><strong>{user['balance']:,.0f} so'm</strong>
            </div>
        </div>
        <div class="card" style="margin-top:2rem;">
            <div class="card-header"><h2>Xaridlar</h2></div>
            <div class="table-wrap"><table>
                <thead><tr><th>#</th><th>Nomi</th><th>Narx</th><th>Sana</th><th>Status</th></tr></thead>
                <tbody>{pur_html or '<tr><td colspan="5" style="text-align:center;color:var(--text-dim);">Xaridlar yoq</td></tr>'}</tbody>
            </table></div>
        </div>
    </div>'''
    return render_page(content, logged_in=True, is_admin=session.get('is_admin', False))
...
def support():
    ...
    list_html = ''
    for t in tickets:
        bc = 'badge-open' if t['status'] == 'open' else ('badge-answered' if t['status'] == 'answered' else 'badge-closed')
...
@app.route('/support/upload_file', methods=['POST'])
@login_required
def support_upload_file():
    ...
    # Fayl hajmini tekshirish (15MB)
    file.seek(0, 2)
    file_size = file.tell()
    file.seek(0)
    if file_size > 15 * 1024 * 1024:
        return jsonify(success=False, message="Fayl hajmi 15MB dan oshmasligi kerak!")

    if not file or not file.filename:
        return jsonify(success=False, message="Fayl tanlanmagan")

    if not allowed_file(file.filename):
        return jsonify(success=False, message="Faqat rasm fayllari: png, jpg, jpeg, gif, webp")

    filename = secure_filename(f"ticket_{ticket_id}_{secrets.token_hex(6)}_{file.filename}")
    file_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)
    file.save(file_path)

    # Xabar matni bo'sh bo'lsa fayl nomini yoz
    if not message_text.strip():
        message_text = f"📎 {file.filename}"

    conn = get_db()
    # Ticket mavjudligini tekshirish
    ...
    conn.execute(
        'INSERT INTO support_messages (ticket_id, user_id, message, file_path, is_admin_reply) VALUES (?, ?, ?, ?, ?)',
        (ticket_id, session['user_id'], message_text, filename,
         1 if session.get('is_admin') else 0)
    )
    conn.execute('UPDATE support_tickets SET status=? WHERE id=?',
                 ('answered' if session.get('is_admin') else 'open', ticket_id))
    conn.commit()
    conn.close()

    return jsonify(success=True, filename=filename, message=message_text)


# ═══════════════════════════════════════════════
# SUPPORT — VIEW TICKET (CHAT)
# ═══════════════════════════════════════════════
...
def view_ticket(ticket_id):
    ...
    # Xabarlarni HTML ga render qilish
    msgs_html = ''
    for m in messages:
        ...
        # Fayl (rasm) ko'rsatish
        file_html = ''
        if m['file_path']:
            file_url = f"/static/uploads/{m['file_path']}"
            file_html = f'<br><a href="{file_url}" target="_blank"><img src="{file_url}" class="chat-img-preview" alt="rasm"></a>'

        msgs_html += f'''<div class="{cls}">
            <div class="msg-avatar {av}">{al}</div>
            <div>
                <div class="msg-bubble">{sanitize(m["message"])}{file_html}</div>
                <div class="msg-meta">{at}{sanitize(m["username"])} • {str(m["created_at"])[:16]}</div>
            </div>
        </div>'''

    bc = 'badge-open' if ticket['status'] == 'open' else ('badge-answered' if ticket['status'] == 'answered' else 'badge-closed')
    ...
    <div class="container" style="max-width:780px;margin:0 auto;padding-top:2rem;">
        <div class="card" style="display:flex;flex-direction:column;">
            <div class="chat-header">
                <div class="chat-header-left">
                    ...
                    <div>
                        ...
                        <span class="badge {bc}" style="font-size:.7rem;">{lb}</span>
                    </div>
                </div>
            ...
            <div class="chat-input-area">
                ...
                <label class="attach-btn" title="Rasm yuborish (max 15MB)">
                    <i class="fas fa-paperclip"></i>
                    <input type="file" id="fileInput" accept="image/png,image/jpg,image/jpeg,image/gif,image/webp">
                </label>
            ...
            <p style="font-size:.72rem;color:var(--text-dim);margin-top:.4rem;text-align:center;">
                <i class="fas fa-image"></i> Rasm yuborish uchun 📎 bosing (max 15MB: png, jpg, gif, webp)
            </p>
        </div>
    </div>
    ...
    (function(){{
        ...
        const UNAME = '{sanitize(session.get("username", ""))}';
        ...
        function scrollBot() {{ area.scrollTop = area.scrollHeight; }}
        scrollBot();
        ...
        function appendMsg(d, mine) {{
            const cls = mine ? 'msg mine' : 'msg';
            const av = mine ? 'user-av' : 'admin-av';
            const al = (d.username || UNAME || '?')[0].toUpperCase();
            ...
            let fileHtml = '';
            if (d.file_path) {{
                const fileUrl = '/static/uploads/' + d.file_path;
                fileHtml = '<br><a href="' + fileUrl + '" target="_blank"><img src="' + fileUrl + '" class="chat-img-preview" alt="rasm"></a>';
            }}
            ...
            div.innerHTML =
                '<div class="msg-avatar ' + av + '">' + al + '</div>' +
                '<div>' +
                    '<div class="msg-bubble">' + (d.message || '') + fileHtml + '</div>' +
                    '<div class="msg-meta">' + at + (d.username || UNAME) + ' • ' + time + '</div>' +
                '</div>';
        ...
        fileInp.addEventListener('change', async function() {{
            ...
            if (file.size > 15 * 1024 * 1024) {{
                showToast('Fayl hajmi 15MB dan katta!', 'error');
                this.value = '';
                return;
            }}
            ...
            if (!allowed.includes(file.type)) {{
                showToast('Faqat rasm fayllari (png, jpg, gif, webp)!', 'error');
                this.value = '';
                return;
            }}
            ...
            formData.append('message', inp.value.trim() || '📎 ' + file.name);

            try {{
                const r = await fetch('/support/upload_file', {{
                    method: 'POST',
                    body: formData
                }});
                const j = await r.json();
                ...
                if (j.success) {{
                    showToast('Rasm yuborildi!', 'success');
                    // Optimistic UI — rasmni darhol ko'rsat
                    appendMsg({{
                        username: UNAME,
                        message: j.message || '📎 ' + file.name,
                        file_path: j.filename,
                        is_admin_reply: IS_ADMIN === 'true'
                    }}, true);
                    scrollBot();
                    lastCount++;
                }} else {{
                    showToast(j.message || 'Yuborishda xatolik!', 'error');
                }}
            }} catch(e) {{
                uploadProgress.classList.remove('show');
...
def admin_panel():
    ...
    pending = conn.execute(
        "SELECT bd.*, u.username as uname, u.minecraft_nick as mc FROM balance_deposits bd JOIN users u ON bd.user_id=u.id WHERE bd.status='pending' ORDER BY bd.created_at DESC"
    ).fetchall()
    total_users = conn.execute('SELECT COUNT(*) as c FROM users WHERE is_admin=0').fetchone()['c']
...
def admin_deposits():
    ...
    <div class="container" style="padding-top:2rem;">
        ...
        <div class="card"><div class="table-wrap"><table>
            ...
            <tbody>{rows or "<tr><td colspan='7' style='text-align:center;color:var(--text-dim);padding:1.5rem;'>Ma'lumotlar yo'q</td></tr>"}</tbody>
        </table></div></div>
    </div>'''
    return render_page(content, logged_in=True, is_admin=True)
...
def admin_support_list():
    ...
    tickets = conn.execute(
        "SELECT st.*, u.username as uname FROM support_tickets st JOIN users u ON st.user_id=u.id ORDER BY CASE WHEN st.status='open' THEN 0 WHEN st.status='answered' THEN 1 ELSE 2 END, st.created_at DESC"
    ).fetchall()
    conn.close()
    lh = ''
    for t in tickets:
    ...
    if not lh:
        lh = '<div style="text-align:center;color:var(--text-dim);padding:2.5rem;"><i class="fas fa-check-circle" style="font-size:2rem;color:var(--success);margin-bottom:.6rem;display:block;"></i>Barcha murojaatlar hal qilingan!</div>'
    content = f'''
...
def admin_ranks():
    content = """
    <div class="container" style="padding-top:2rem;">
        <div class="section-title"><h2>👑 Rank Management</h2></div>
        <div style="display:flex;gap:1rem;margin-bottom:2rem;flex-wrap:wrap;">
            <button onclick="showAddModal()" class="btn btn-primary"><i class="fas fa-plus-circle"></i> Yangi Rank</button>
            <button onclick="bulkActivate()" class="btn btn-secondary"><i class="fas fa-check-double"></i> Barchasini Aktiv</button>
            <button onclick="bulkDeactivate()" class="btn btn-outline"><i class="fas fa-ban"></i> Barchasini O'chir</button>
            <button onclick="exportRanks()" class="btn btn-outline"><i class="fas fa-download"></i> Export JSON</button>
        </div>
        <div class="card">
            <div class="card-header"><i class="fas fa-filter"></i><h2>Filter</h2></div>
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:1rem;">
                <input type="text" id="searchBox" placeholder="Qidiruv..." oninput="filterRanks()" style="padding:.8rem;border-radius:8px;border:1px solid var(--border);background:rgba(255,255,255,.05);color:var(--text);">
                <select id="categoryFilter" onchange="filterRanks()" style="padding:.8rem;border-radius:8px;border:1px solid var(--border);background:rgba(255,255,255,.05);color:var(--text);">
                ...
                <select id="statusFilter" onchange="filterRanks()" style="padding:.8rem;border-radius:8px;border:1px solid var(--border);background:rgba(255,255,255,.05);color:var(--text);">
    ...
    <div id="rankModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.9);backdrop-filter:blur(10px);z-index:10000;padding:2rem;overflow-y:auto;align-items:flex-start;justify-content:center;">
        <div style="max-width:800px;width:100%;margin:0 auto;background:var(--bg-secondary);border-radius:20px;border:2px solid var(--primary);box-shadow:0 20px 60px rgba(0,255,136,.3);">
            <div style="display:flex;justify-content:space-between;align-items:center;padding:2rem;border-bottom:2px solid var(--border);">
                ...
                <button onclick="closeModal()" style="background:none;border:none;color:var(--danger);font-size:1.5rem;cursor:pointer;"><i class="fas fa-times"></i></button>
            </div>
            <form id="rankForm" onsubmit="return handleSubmit(event)" style="padding:2rem;">
                <input type="hidden" id="rankId">
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-bottom:1.5rem;">
                    <div class="form-group"><label>Rank Nomi *</label><input type="text" id="rankName" required placeholder="VIP, Premium..."></div>
                ...
                <div class="form-group" style="margin-bottom:1.5rem;"><label>Tavsif</label><textarea id="rankDesc" rows="3" placeholder="Rank haqida..."></textarea></div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-bottom:1.5rem;">
                    <div class="form-group"><label>Narxi (so'm) *</label><input type="number" id="rankPrice" required placeholder="10000" min="0"></div>
                    <div class="form-group"><label>Davomiyligi *</label><input type="text" id="rankDuration" required placeholder="30, 90, UMRBOT"></div>
                </div>
                <div class="form-group" style="margin-bottom:1.5rem;"><label>Imkoniyatlar (vergul bilan)</label><textarea id="rankFeatures" rows="3" placeholder="/fly, /feed, Kit Premium"></textarea></div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-bottom:2rem;">
                    <div class="form-group"><label>Rang *</label>
                        <div style="display:flex;gap:.5rem;align-items:center;">
                            ...
                            <input type="text" id="rankColor" value="#3b82f6" placeholder="#3b82f6">
                        </div>
                    </div>
                ...
                <div style="display:flex;gap:1rem;padding-top:1.5rem;border-top:2px solid var(--border);">
                    <button type="submit" class="btn btn-primary" style="flex:1;"><i class="fas fa-save"></i> Saqlash</button>
                    <button type="button" onclick="closeModal()" class="btn btn-outline" style="flex:1;"><i class="fas fa-times"></i> Bekor</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
    const categoryNames = {anarchy:'⚔️ Anarxiya',smp:'🌲 SMP',keys:'🔑 Kalitlar',services:'🛠️ Xizmatlar',token:'🪙 Tokenlar'};
    let allPackages = [];
    let editMode = false;

    document.getElementById('rankColorPicker').addEventListener('input', e => document.getElementById('rankColor').value = e.target.value);
    document.getElementById('rankColor').addEventListener('input', e => { if(/^#[0-9A-F]{6}$/i.test(e.target.value)) document.getElementById('rankColorPicker').value = e.target.value; });
    ...
    function renderRanks() {
        ...
        for (const [category, ranks] of Object.entries(categories)) {
            html += `<div class="card" style="margin-bottom:2rem;border-left:4px solid var(--primary);">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem;">
                    <h3 style="color:var(--primary);font-family:'Orbitron',sans-serif;">${categoryNames[category]||category}</h3>
                    <span style="color:var(--text-dim);">${Object.keys(ranks).length} rank</span>
                </div>`;
            for (const [rankName, variants] of Object.entries(ranks)) {
                const base = variants[0];
                const features = base.features ? base.features.split(',') : [];
                html += `<div style="border:2px solid ${base.color};border-radius:12px;padding:1.5rem;margin-bottom:1.5rem;background:rgba(0,0,0,.2);">
                    <div style="font-size:1.8rem;font-weight:900;color:${base.color};font-family:'Orbitron',sans-serif;">${rankName}</div>
                    <p style="color:var(--text-dim);font-size:.9rem;margin:.5rem 0;">${base.description||''}</p>`;
                if (features.length) {
                    html += `<div style="display:flex;flex-wrap:wrap;gap:.4rem;margin-bottom:1rem;">${features.map(f=>`<span style="background:rgba(34,197,94,.2);color:#22c55e;padding:.2rem .7rem;border-radius:20px;font-size:.8rem;border:1px solid #22c55e;"><i class="fas fa-check"></i> ${f.trim()}</span>`).join('')}</div>`;
                }
                html += `<div style="display:grid;gap:.8rem;">`;
                variants.forEach(v => {
                    html += `<div style="background:rgba(0,217,255,.05);padding:1rem 1.5rem;border-radius:8px;border:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;">
                        <div style="display:flex;gap:2rem;align-items:center;flex:1;">
                            <div style="color:var(--accent);min-width:120px;"><i class="fas fa-clock"></i> ${v.duration}</div>
                            <div style="font-weight:700;font-size:1.3rem;color:${v.color};">${v.price.toLocaleString()} <span style="font-size:.8rem;opacity:.8;">so'm</span></div>
                            <span class="badge ${v.is_active?'badge-success':'badge-danger'}">${v.is_active?'✅ Aktiv':'❌ Nofaol'}</span>
                        </div>
                        <div style="display:flex;gap:.5rem;">
                            <button onclick="editRank(${v.id})" class="btn btn-sm btn-outline" title="Tahrirlash"><i class="fas fa-edit"></i></button>
                            <button onclick="deleteRank(${v.id},'${v.name}','${v.duration}')" class="btn btn-danger btn-sm" title="O'chirish"><i class="fas fa-trash"></i></button>
                            <button onclick="duplicateRank(${v.id})" class="btn btn-secondary btn-sm" title="Nusxa"><i class="fas fa-copy"></i></button>
                        </div>
                    </div>`;
                });
                html += `</div></div>`;
        ...
        container.innerHTML = html || '<div class="card"><p style="text-align:center;color:var(--text-dim);padding:3rem;">Rank topilmadi</p></div>';
    }
    ...
    function showAddModal() {
        editMode = false;
        document.getElementById('modalTitle').innerHTML = '<i class="fas fa-plus-circle"></i> Yangi Rank';
    ...
    async function handleSubmit(e) {
        ...
        if (result.success) { closeModal(); loadRanks(); showToast(result.message,'success'); }
        else showToast('Xatolik: '+(result.message||'Noma\\'lum'),'error');
        return false;
    }
    ...
    async function deleteRank(id, name, duration) {
        ...
        const result = await res.json();
        if (result.success) { loadRanks(); showToast(result.message,'success'); }
        else showToast('Xatolik!','error');
    }

    async function duplicateRank(id) {
        const pkg = allPackages.find(p => p.id === id);
        if (!pkg || !confirm(`"${pkg.name}" nusxalansinmi?`)) return;
        const data = {...pkg, name: pkg.name+' (Copy)', is_active: 0};
        const res = await fetch('/admin/add_rank', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});
        const result = await res.json();
        if (result.success) { loadRanks(); showToast('Nusxalandi!','success'); }
    }

    async function bulkActivate() {
        if (!confirm('Barchasini aktivlashtirish?')) return;
        for (const pkg of allPackages) {
            if (!pkg.is_active) await fetch(`/admin/edit_rank/${pkg.id}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...pkg,is_active:1})});
        }
        loadRanks(); showToast('Barchasi aktivlashtirildi!','success');
    }

    async function bulkDeactivate() {
        if (!confirm("Barchasini o'chirish?")) return;
        for (const pkg of allPackages) {
            if (pkg.is_active) await fetch(`/admin/edit_rank/${pkg.id}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...pkg,is_active:0})});
        }
        loadRanks(); showToast("Barchasi o'chirildi!",'success');
    }

    function exportRanks() {
        const dataUri = 'data:application/json;charset=utf-8,'+ encodeURIComponent(JSON.stringify(allPackages,null,2));
        const a = document.createElement('a');
        a.setAttribute('href',dataUri);
        a.setAttribute('download','ranks_'+new Date().toISOString().slice(0,10)+'.json');
        a.click();
        showToast('Export qilindi!','success');
    }

    document.addEventListener('keydown', e => { if(e.key==='Escape') closeModal(); });
    document.getElementById('rankModal').addEventListener('click', e => { if(e.target.id==='rankModal') closeModal(); });
    loadRanks();
    </script>"""
    return render_page(content, logged_in=True, is_admin=True)
...
def add_rank():
    try:
        ...
        conn = get_db()
        conn.execute("INSERT INTO packages (category,name,description,price,duration,features,color,is_active) VALUES (?,?,?,?,?,?,?,?)",
...
def admin_news():
    ...
    rows = ''
    for n in all_news:
        rows += f'<tr><td>#{n["id"]}</td><td><strong>{sanitize(n["title"])}</strong></td><td>{str(n["created_at"])[:16]}</td><td><button onclick="deleteNews({n["id"]})" class="btn btn-danger btn-sm"><i class="fas fa-trash"></i></button></td></tr>'
    content = f'''
    <div class="container" style="padding-top:2rem;max-width:900px;">
        <div class="section-title"><h2>📰 Yangiliklar Boshqaruvi</h2></div>
        <div class="tabs"><a href="/admin" class="tab"><i class="fas fa-tachometer-alt"></i> Dashboard</a><a href="/admin/news" class="tab active"><i class="fas fa-newspaper"></i> Yangiliklar</a></div>
        <div class="card">
            <div class="card-header"><i class="fas fa-plus"></i><h2>Yangilik Qo'shish</h2></div>
            <form id="addNewsForm" enctype="multipart/form-data">
                <div class="form-group"><label>Sarlavha</label><input type="text" name="title" required placeholder="Yangilik sarlavhasi..."></div>
                <div class="form-group"><label>Matn</label><textarea name="content" rows="5" required placeholder="Yangilik matni..."></textarea></div>
                <div class="form-group"><label>Rasm (ixtiyoriy)</label><div class="file-upload"><input type="file" name="image" accept="image/*"><div class="file-upload-label"><i class="fas fa-image"></i><p>Rasm tanlash</p></div></div></div>
                <button type="submit" class="btn btn-primary"><i class="fas fa-plus"></i> Qo'shish</button>
            </form>
        </div>
        <div class="card">
            <div class="card-header"><i class="fas fa-list"></i><h2>Barcha Yangiliklar</h2></div>
            <div class="table-wrap"><table>
                <thead><tr><th>#</th><th>Sarlavha</th><th>Sana</th><th>Amal</th></tr></thead>
                <tbody>{rows or "<tr><td colspan='4' style='text-align:center;color:var(--text-dim);'>Yangiliklar yo'q</td></tr>"}</tbody>
            </table></div>
        </div>
    </div>
    <script>
    async function deleteNews(id){{
        ...
        const r = await fetch('/admin/news/delete/'+id, {{method:'POST'}});
        const j = await r.json();
        showToast(j.message, j.success?'success':'error');
        if(j.success) setTimeout(()=>location.reload(), 1200);
    }}
            ...
            <div class="card">
                ...
                <div class="form-group"><label>Trayler ko'rsatilsinmi?</label>
                    <select name="show_trailer" form="settingsForm">
                        ...
                        <option value="0" {'selected' if settings.get('show_trailer') != '1' else ''}>Yo'q</option>
                    </select>
                </div>
            ...
            <div class="card">
                ...
                <div class="form-group"><label>Musiqa yoqilgan?</label>
                    <select name="music_enabled" form="settingsForm">
                        ...
                        <option value="0" {'selected' if settings.get('music_enabled') != '1' else ''}>Yo'q</option>
                    </select>
                </div>
            </div>
        </form>
        <button onclick="saveSettings()" class="btn btn-primary btn-full" style="margin-bottom:3rem;">
...
def update_player_stats():
    ...
    try:
        ...
        conn.execute('''INSERT INTO player_stats (minecraft_nick,server_type,kills,deaths,time_played,money)
                        ...
                        DO UPDATE SET kills=?,deaths=?,time_played=?,money=?,last_updated=CURRENT_TIMESTAMP''',
                     (nick, srv, kills, deaths, time_played, money, kills, deaths, time_played, money))
        conn.commit()
        conn.close()
...
if not os.path.exists('elitemc.db'):
    ...
    # Mavjud DB ga migration (file_path ustuni)
    try:
        conn = get_db()
        conn.execute('ALTER TABLE support_messages ADD COLUMN file_path TEXT')
        conn.commit()
        conn.close()
        print("✅ Migration: file_path ustuni qo'shildi")
    except Exception:
...
if __name__ == '__main__':
    ...
    print(f"  📎 Max fayl     : 15MB (png,jpg,gif,webp)")
    print("=" * 62)
